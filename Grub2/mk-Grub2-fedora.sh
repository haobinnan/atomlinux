...
AtomLinux_DownloadURL="https://github.com/rhboot/grub2/archive/"
AtomLinux_Grub2PrefixDirName="$(grep -i ^AtomLinux_Grub2PrefixDirName ../VariableSetting | cut -f2 -d'=')"
AtomLinux_Grub2VNumber="fedora-29"
AtomLinux_Grub2LdrName="$(grep -i ^AtomLinux_Grub2LdrName ../VariableSetting | cut -f2 -d'=')"
AtomLinux_UsingPreviousBuildResults_SecureBoot="No"
#Load from VariableSetting file

OBJ_PROJECT=grub2
FILENAME_DIR=grub2-$AtomLinux_Grub2VNumber
...
if [ $CodeAcquisitionMethod = "wget" ]; then
    ...
    if [ ! -f ./${FILENAME} ]; then
        ...
        #Check if necessary tools are installed
        wget ${AtomLinux_DownloadURL}${AtomLinux_Grub2VNumber}.tar.gz -O ${FILENAME}
        if [ ! $? -eq 0 ]; then
...
#Check make install
sudo chmod -R 777 ${CurrentDIR}/${OBJ_PROJECT}-tmp"/install_tmp"
...
function build_efi()
{
    ...
    # --disable-werror
    ./configure --with-platform=efi --target=${ARCH} --prefix=${CurrentDIR}/${OBJ_PROJECT}-tmp"/install_tmp" --disable-grub-mount --disable-werror
    #Check configure
    if [ ! $? -eq 0 ]; then
        echo "Error: configure (Grub2) ."
        exit 1
    ...
    # $(ls *.mod | cut -d '.' -f 1)
    #All mod
    # reboot multiboot linuxefi
    ../../../bin/grub-mkimage -O ${ARCH}-efi -d . -o ${LDRNAME} -p ${AtomLinux_Grub2PrefixDirName} newc memdisk cpio part_gpt part_msdos msdospart ntfs ntfscomp fat exfat normal chain boot configfile linux png all_video search blocklist iso9660 udf minicmd loopback gfxmenu gfxterm romfs procfs sleep ls cat echo halt test linux cpuid scsi lsefi lsefimmap efifwsetup efinet backtrace font loadenv syslinuxcfg video
    #Check grub-mkimage
    if [ ! $? -eq 0 ]; then
        echo "Error: grub-mkimage (Grub2) ."
        exit 1
